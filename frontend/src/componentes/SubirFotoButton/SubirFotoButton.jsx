  ...
  const handleChange = async (info) => {
    ...
    if (info.file.status === 'done') {
      if (info.file.response.status) {
        ...
        setAvatarUser(`${API_URL}/api/user/fotoPerfil/${info.file.response.user.id}/?timestamp=${new Date().getTime()}`);
    ...
    <Modal 
      ...
      onCancel={handleCancel}>
        ...
        <ImgCrop rotationSlider>
            ...
            {(errors && errors.fotoPerfil) ? errors.fotoPerfil.map((error, index) => <p className='text-danger error' key={index}>{error}</p>) : ''}  
      </Modal>
    </>
  );
};
